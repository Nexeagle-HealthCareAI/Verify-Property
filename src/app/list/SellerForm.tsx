    ...
    return (
      <div style={{ textAlign: "center", padding: "var(--space-8)", background: "var(--surface-light)", borderRadius: "var(--radius-lg)" }}>
        ...
        <p style={{ color: "var(--color-text-muted)" }}>
          Our verification team will contact you shortly to process your listing.
        </p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
      ...
      <div>
        ...
        <select id="propertyType" className="form-input" {...register("propertyType")}>
          ...
          <option value="Ready House">Ready House</option>
          <option value="Flat/Apartment">Flat/Apartment</option>
        </select>
        {errors.propertyType && <p style={{ color: "var(--color-danger)", fontSize: "var(--text-xs)", marginTop: 4 }}>{errors.propertyType.message}</p>}
      </div>

      <div>
        <label className="form-label" htmlFor="location">General Location</label>
        <input id="location" type="text" className="form-input" placeholder="e.g. Bahadurganj Road, Kishanganj" {...register("location")} />
        {errors.location && <p style={{ color: "var(--color-danger)", fontSize: "var(--text-xs)", marginTop: 4 }}>{errors.location.message}</p>}
      </div>
